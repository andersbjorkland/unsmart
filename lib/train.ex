defmodule Unsmart.Train do
  ...
  def model() do
    batches = nil
    features = 3
    model = Axon.input("input", shape: {batches, features}) |> Axon.dense(10, activation: :tanh) |> Axon.dense(4, activation: :softmax)
    model
  end
  ...
  def train() do
    ...
    fizz_model = model()
    loop = Axon.Loop.trainer(fizz_model, :categorical_cross_entropy, Polaris.Optimizers.adamw(learning_rate: 0.05))
    ...
    convert = fn {label, index} -> ~w[fizz buzz fizzbuzz] |> Enum.at(label) |> Kernel.||(index) end
    Axon.predict(fizz_model, params, test_data()) |> Nx.squeeze() |> Nx.argmax(axis: 1) |> Nx.to_flat_list |> Enum.with_index(1) |> Enum.map(convert)
  end
